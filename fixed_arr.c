...
// Contains references to all children. If the pointer is NULL it does not exist.
struct trie_tree {
...
void get_specific(struct trie_tree *root, struct trie_tree **child, char *element) {
  size_t idx = char_to_index(*element);
  *child = root->children[idx];
}
